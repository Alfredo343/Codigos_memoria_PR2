...
# ------------------ CONFIG (FRESA1 - BOTELLAS) ------------------
MECHANISM_NAME   = 'cinta_fresa1'
BOTTLE_NAME      = 'bottle_fresa'
...
# ------------------ SINCRONIZACIÓN con FRESA2/ROBOT ------------------
PARAM_SENSOR_BFRESA   = 'sensor_bfresa'     # 0 normal, 1 pick, 2 avance, 3 place, 4 reset
...
if not (mechanism.Valid() and botella_orig.Valid() and padre_mov.Valid()):
    raise RuntimeError("Faltan ítems de FRESA1 (cinta/botella/MOV).")

# Posición inicial del eje
try:
...
# Para congelar el tiempo cuando FRESA2/ROBOT bloquea
bloqueada = False
t_block_start = 0.0

cola = []  # {'item': Item, 'spawn_pos': float, 'contada': bool, 'llenada': bool}
contador = 0
...
# Asegurar que existe el contador global
if RDK.getParam(PARAM_CONTADOR_FRESA) is None:
    set_int_param(PARAM_CONTADOR_FRESA, 0)
...
def resetear_eje_si_toca():
    """
    Reset SIN salto visual:
    - Resetea el eje
    - Ajusta spawn_pos de todas las botellas
    - Recoloca su Pose para que no haya hueco
    """
    ...
    if RESET_EJE and pos_cinta > RESET_EJE:
        ...
        # Reset físico del mecanismo
        try:
            mechanism.setJoints([0])
        except:
            mechanism.MoveJ([0])

        # Ajustar spawn_pos + recolocar todas las botellas para continuidad visual
        for e in cola:
            if not e['item'].Valid():
                continue
            e['spawn_pos'] -= delta
...
# ------------------ main ------------------
print("--- FRESA1 ACTIVA (con RESET corregido) ---")

try:
    while True:
        limpiar_invalidas()

        # -------- INTERLOCK con FRESA2/ROBOT --------
        estado_b = get_int_param(PARAM_SENSOR_BFRESA, 0)

        # Bloqueo cuando el robot está trabajando (1 o 3)
        if estado_b in [1, 3]:
            if not bloqueada:
                bloqueada = True
        ...
        if estado_cinta == "RUN":
            ...
            # Control máximo de botellas en escena
        ...
        # 4) Eliminación fin cinta + contador_fresa (+1 por botella)
        while cola:
            first = cola[0]
    ...
    print("FRESA1 detenido por usuario.")
except Exception as e:
    print("ERROR en FRESA1:", e)
finally:
    RDK.setParam('sensor_zfresa', '0')
    print("--- FRESA1 FINALIZADA ---")
